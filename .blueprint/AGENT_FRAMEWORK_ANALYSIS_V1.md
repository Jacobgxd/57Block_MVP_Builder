...
- 专业 Agent 默认采用 `Full Runtime`：`think -> plan -> execute -> reflect`
- `Orchestrator` 默认采用 `Lite Runtime`：`analyze -> decide -> act -> verify`
- 框架需要允许保留结构化 Runtime 摘要，但不强制暴露原始完整思维链
- 框架需要允许把 `runtime.phase_trace / task_complexity / plan_level` 作为可持久化、可观测的业务字段
...
在 `PRD_V1.md` v1.21 之后，还要额外满足第四个特征：

4. **统一 Runtime 协议**：专业 Agent 与 `Orchestrator` 采用不同轻重的运行闭环，并要求可解释摘要与结构化回传
...
- Runtime 协议层（`Full Runtime / Lite Runtime`、可见摘要、`runtime` 外壳）
- 上下文装配器（按场景分层装配策略）
- MCP 工具封装层
...
| 版本 | 日期 | 变更说明 |
|------|------|---------|
| V1.1 | 2026-03-30 | 对齐 `PRD_V1.md` v1.21，补充统一 Runtime 协议需求，明确底层框架除状态、Guardrails、Memory 外还需承载 `Full Runtime / Lite Runtime` 与结构化 `runtime` 外壳 |
