...
- 状态消息说明“系统正在做什么”（特别是在触发 Mockup Agent 等不直接输出 user_visible_reply 的纯后台执行任务时，Orchestrator 必须负责生成过渡性的状态提示，填补对话区空白）
...
- 当检测到资产正文在 AI 生成/更新期间被用户手动修改时（如手动保存了新版本或进行了外部编辑）
- 暂停 AI 的自动覆盖写入
- 触发合并确认，提示用户保留最新手动版本还是应用 AI 生成版本
