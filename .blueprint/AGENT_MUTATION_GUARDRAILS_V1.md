...
从 `PRD_V1.md` v1.21 起，所有 Agent 的正式动作还必须同时适配统一 Runtime：

- 专业 Agent 默认采用 `Full Runtime`
- `Orchestrator` 默认采用 `Lite Runtime`
- 正式生成 / 更新 / 重生成 / 审查 / 确认前准备，不得跳过最小 Runtime 闭环
- 对用户展示的是 Runtime 摘要，而不是原始完整思维链

### 2.5 文档型资产与执行型资产可以采用不同落地方式
...
进入以下判断前，专业 Agent 必须先完成最小 `Think`，再决定是否进入 `Plan / Execute / Reflect`。

对于 `Orchestrator`，必须先完成最小 `Analyze`，再决定是否进入 `Decide / Act / Verify`。
...
### 8.7 用户强制放行规则 (Override)

为了防止 AI 自检出现“假阳性”导致流程死锁：

- 当 Agent 认为存在 `blocking_gap` 拒绝进入 `confirmable` 状态时
- 若用户在对话中明确回复“我确认忽略此风险，强制确认基线”或类似强指令
- `Orchestrator` 应允许放行，并在 `confirmation_log` 中将这些未决风险显式记录为 `owner_skipped_items`
...
- 是否需要在 `Reflect` 阶段回传风险或 `handoff_request`
...
| 版本 | 日期 | 变更说明 |
|------|------|---------|
| V1.2 | 2026-03-30 | 对齐 `PRD_V1.md` v1.21，补充统一 Runtime 底线、`runtime` 结构化字段、最小 `Think / Analyze` 前置要求，以及各 Agent 在 Runtime 下的自主执行边界 |
